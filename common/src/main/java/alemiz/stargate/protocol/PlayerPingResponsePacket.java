...
@ToString
public class PlayerPingResponsePacket extends StarGatePacket {

    private String playerName;
    private long upstreamPing;
    private long downstreamPing;

    @Override
    public void encodePayload(ByteBuf byteBuf) {
        PacketHelper.writeString(byteBuf, this.playerName);
        PacketHelper.writeLong(byteBuf, this.upstreamPing);
        PacketHelper.writeLong(byteBuf, this.downstreamPing);
    }

    @Override
    public void decodePayload(ByteBuf byteBuf) {
        this.playerName = PacketHelper.readString(byteBuf);
        this.upstreamPing = PacketHelper.readLong(byteBuf);
        this.downstreamPing = PacketHelper.readLong(byteBuf);
    }
