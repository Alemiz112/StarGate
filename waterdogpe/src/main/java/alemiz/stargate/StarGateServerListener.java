...
public class StarGateServerListener extends alemiz.stargate.server.StarGateServerListener {
    ...
    public boolean onSessionCreated(InetSocketAddress address, ServerSession session) {
        ...
        this.loader.getProxy().getEventManager().callEvent(event);
    ...
    public void onSessionAuthenticated(ServerSession session) {
        ...
        this.loader.getProxy().getEventManager().callEvent(event).whenComplete((eventFuture, error) ->
                this.onAuthenticatedEvent(event, error));
    }

    private void onAuthenticatedEvent(ClientAuthenticatedEvent event, Throwable error){
        if (error != null){
            this.loader.getStarGateLogger().error("While client authentication exception was thrown!", error);
            event.getSession().disconnect("Authentication failed!");
            return;
        }

        if (event.isCancelled()){
            event.getSession().disconnect(event.getCancelMessage());
            return;
        }

        if (this.loader.isCheckClientNames()){
            ServerSession oldSession = this.loader.getSession(event.getSession().getSessionName());
            if (oldSession != null && event.getSession() != oldSession){
